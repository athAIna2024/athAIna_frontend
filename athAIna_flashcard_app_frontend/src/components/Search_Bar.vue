...
const handleKeyPress = async (event) => {
  ...
  flashcardStore.setSearchResults(searchResults.value);
};
...
<template>
  <div class="athAIna-border-outer p-1 rounded-[20px] h-[40px]">
    ...
    <div v-if="flashcardStore.searchResults.length">
      <ul>
        <li v-for="result in flashcardStore.searchResults" :key="result.id">{{ result.title }}</li>
      </ul>
    </div>
  </div>
</template>
