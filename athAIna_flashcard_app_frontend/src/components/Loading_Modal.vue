...
watchEffect(() => {
  if (props.isVisible) {
    ...
    timeoutId = setTimeout(() => {
      if (props.condition === true) { // Condition that has to be met to close loading modal
        emit("close");
      }
    }, 3000);
  }
  ...
  <div v-if="isVisible" class="fixed inset-0 flex items-center justify-center bg-[rgba(0,0,0,0.5)] bg-opacity-50 z-40">
    <div class="athAIna-border-outer p-1 flex flex-col w-[350px]">
      <div class="athAIna-border-inner px-10 py-10 text-center">
        ...
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" class="p-0" width="" height="50">
          ...
          </radialGradient><circle transform-origin="center" fill="none" stroke="url(#a12)" stroke-width="10" stroke-linecap="round" stroke-dasharray="200 1000" stroke-dashoffset="0" cx="100" cy="100" r="70">
          <animateTransform type="rotate" attributeName="transform" calcMode="spline" dur="2" values="360;0" keyTimes="0;1" keySplines="0 0 1 1" repeatCount="indefinite"></animateTransform>
        </circle><circle transform-origin="center" fill="none" opacity=".2" stroke="#FFBA20" stroke-width="10" stroke-linecap="round" cx="100" cy="100" r="70"></circle>
        </svg>
